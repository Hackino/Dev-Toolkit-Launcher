...
import { detectVariants } from '../process/variantDetection';
import { MobileConfigRepository } from '../db/MobileConfigRepository';
import { FirebaseConfigRepository } from '../db/FirebaseConfigRepository';
import { MobileBuildHistoryRepository } from '../db/MobileBuildHistoryRepository';
...
import {
  runMobileTask,
  stopMobileTask,
} from '../process/mobile-process';
import { resolveEnvFlags } from '../process/buildFlagResolver';
import {
  listAndroidDevices,
  ...
  listIosSimulators,
  listFlutterDevices,
} from '../process/devices';
import { readGradleVersion, writeGradleVersion } from '../process/version/gradleVersion';
import { readPlistVersion, writePlistVersion } from '../process/version/plistVersion';
import { readPubspecVersion, writePubspecVersion } from '../process/version/pubspecVersion';
import { readGradlePropertiesVersion, writeGradlePropertiesVersion } from '../process/version/gradlePropertiesVersion';
...
export function registerMobileIpc(): void {
  ...
  ipcMain.handle('mobile:build', (_e, args: MobileBuildArgs) => {
    try {
      ...
      const strategy = StrategyRegistry.getMobile(project.type as MobilePlatform);
