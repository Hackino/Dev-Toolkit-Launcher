...
function BackendTypeIcon({ type, color }: { type: BackendProjectType; color: string }): JSX.Element | null {
  switch (type) {
    ...
    case 'nextjs':
      return (
        <path
          d="M17 34V14l16 20M31 14v12"
          stroke={color} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" fill="none"
        />
      );
    case 'express':
      return (
        <text x="24" y="31" textAnchor="middle" fill={color} fontSize="17" fontWeight="700" fontFamily="system-ui, sans-serif">ex</text>
      );
    default:
      return null;
  }
...
/** A rounded-square badge with custom inner art. */
function Badge({ bg, children }: { bg: string; children: JSX.Element }): JSX.Element {
  return (
...
function KtorBadge(): JSX.Element {
  const id = useId();
  return (
    <svg width="38" height="38" viewBox="0 0 48 48" fill="none" className="column-type-logo">
      <defs>
        <linearGradient id={id} x1="9" y1="9" x2="39" y2="39" gradientUnits="userSpaceOnUse">
          <stop offset="0" stopColor="#16C2F3" />
          <stop offset="0.5" stopColor="#8A5BFF" />
          <stop offset="1" stopColor="#FF8A00" />
        </linearGradient>
      </defs>
      <rect width="48" height="48" rx="10" fill="#fff" />
      {/* tilted gradient square */}
      <path d="M24 6.5 41.5 24 24 41.5 6.5 24Z" fill={`url(#${id})`} />
      {/* centered black square */}
      <rect x="18.5" y="18.5" width="11" height="11" fill="#000" />
    </svg>
  );
}
...
export function BackendTypeLogo({ type }: { type: BackendProjectType }): JSX.Element {
  switch (type) {
    case 'spring-boot':
      return (
        <Badge bg="#ffffff">
          <g transform="translate(7.2 7.2) scale(1.4)"><path d={SPRING_LEAF_PATH} fill="#6DB33F" /></g>
        </Badge>
      );
    case 'nestjs':
      return (
        <Badge bg="#ffffff">
          <g transform="translate(7.2 7.2) scale(1.4)"><path d={NEST_LION_PATH} fill="#E0234E" /></g>
        </Badge>
      );
    case 'ktor':
      return <KtorBadge />;
    case 'nodejs':
      return (
        <Badge bg="#83B81A">
          <text x="24" y="29" textAnchor="middle" fill="#3C3C3C" fontSize="12" fontWeight="800" fontFamily="system-ui, sans-serif" letterSpacing="-0.5">node</text>
        </Badge>
      );
    default:
      return (
        <Badge bg={BACKEND_COLORS[type]}>
